 ...
 * Displays category navigation tabs with:
...
const CategoryFilter = () => {
  const { categories, activeCategory, setActiveCategory, allItems, items } = useMenu();

  // Count items per category
  const getCategoryCount = (categoryId: string): number => {
    if (categoryId === 'all') return allItems.length;
    return allItems.filter(item => item.category.toLowerCase() === categoryId.toLowerCase()).length;
  };
  ...
  return (
    <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-15 md:top-18 z-40 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 py-3">
        {/* Category tabs container with horizontal scroll on mobile */}
        ...
        <div className="mt-3 flex items-center justify-between text-sm">
          <p className="text-gray-500 dark:text-gray-400">
            Showing <span className="font-semibold text-gray-700 dark:text-gray-200">{items.length}</span> items
            {activeCategory !== 'all' && (
              <span> in <span className="capitalize font-medium text-orange-500">{activeCategory}</span></span>
            )}
          </p>
        </div>
      </div>

 ...
 * This is re-mounted when key changes, which resets all state
 */
...
const ModalContent = ({ item, onClose, onAddToCart }: ModalContentProps) => {
  ...
  const handleAddToCart = () => {
    ...
    toast.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-[60] animate-fadeIn';
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          {item.spicyLevel > 0 && (
            <div className="flex items-center gap-1 px-3 py-1 bg-red-100 dark:bg-red-900/30 rounded-full">
              <span className="text-sm font-medium text-red-600 dark:text-red-400">Spicy Level:</span>
              <span>{Array.from({ length: item.spicyLevel }, () => '🌶️').join('')}</span>
            </div>
          )}
        </div>
 ...
 * Wrapper that handles visibility and keyboard events
 * Uses key prop on ModalContent to reset state when item changes
 */
